...
if ((!empty($_GET['id']))&&(isset($_POST['oui']))) {
    ...
    $Update=$cnx->prepare("UPDATE ".$Prefix."neuro_Calendrier SET statue=0 WHERE id=:id");
    $Update->bindParam(':id', $Id, PDO::PARAM_INT);
    $Update->execute();

    header('Location:'.$Home.'/Admin/Calendrier/');
}
...
Etes-vous sur de vouloir désactiver cette événement ? <BR /><BR />
