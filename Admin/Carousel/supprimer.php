...
if ((!empty($_GET['id']))&&(isset($_POST['oui']))) {
    ...
    $Select=$cnx->prepare("SELECT (lien) FROM ".$Prefix."neuro_Carousel WHERE id=:id");
    $Select->bindParam(':id', $Id, PDO::PARAM_INT);
    $Select->execute();
    $Lien=$Select->fetch(PDO::FETCH_OBJ);

    unlink($_SERVER['DOCUMENT_ROOT']."/lib/carousel/".basename($Lien->lien));

    $deleteActu=$cnx->prepare("DELETE FROM ".$Prefix."neuro_Carousel WHERE id=:id");
    $deleteActu->bindParam(':id', $Id, PDO::PARAM_INT);
    $deleteActu->execute();

    header('location:'.$Home.'/Admin/Carousel/');
}
