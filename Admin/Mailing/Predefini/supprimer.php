...
if ((!empty($_GET['id']))&&(isset($_POST['oui']))) {
    ...
    $deleteActu=$cnx->prepare("DELETE FROM ".$Prefix."neuro_mailing_Predefini WHERE id=:id");
    $deleteActu->bindParam(':id', $Id, PDO::PARAM_INT);
    $deleteActu->execute();

    header('Location:'.$Home.'/Admin/Mailing/Predefini/');
}
...
Etes-vous sur de vouloir supprimer ce mail ? <BR /><BR />
