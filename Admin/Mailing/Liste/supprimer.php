...
if ((!empty($_GET['id']))&&(isset($_POST['oui']))) {
    ...
    $deleteActu=$cnx->prepare("DELETE FROM ".$Prefix."neuro_mailing_Liste WHERE id=:id");
    $deleteActu->bindParam(':id', $Id, PDO::PARAM_INT);
    $deleteActu->execute();

    header('Location:'.$Home.'/Admin/Mailing/Liste/');
}
...
Etes-vous sur de vouloir supprimer cette email ? <BR /><BR />
